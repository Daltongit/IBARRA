// JS/script-simulador.js - IBARRA (SIMULADOR 3 OK)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function finishMultiPhase() {
        ...
        phase2Blocks.forEach(b => { b.respuestas_bloque.forEach((ans, i) => { const qNum = b.rango_inicio + i; const userArr = tableAnswersImg[qNum] || []; if(JSON.stringify(userArr.sort()) === JSON.stringify(ans.sort())) ok2++; }); });
        let total = phase1Data.length + 20;
        document.getElementById('revision-container').innerHTML = `<h3>Resumen</h3><p>Vocabulario: ${ok1}</p><p>Abstracto: ${ok2}</p>`; showStats(ok1+ok2, total);
    }

    function finishTableImg() {
        let ok = 0; questions.forEach((q, i) => { if(tableAnswersImg[i] === q.respuesta_index) ok++; });
        document.getElementById('revision-container').innerHTML = "<p>Revisión visual finalizada.</p>"; showStats(ok, questions.length);
    }
