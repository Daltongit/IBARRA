...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function render() {
        ...
        users.forEach(user => {
            ...
            let html = '';
            if (intentosUser.length === 0) html = '<p style="text-align:center;padding:15px;color:#999;">Sin intentos.</p>';
            else {
                [...new Set(intentosUser.map(i=>i.materia))].sort().forEach(m => {
                    ...
                    im.forEach(i => {
                        ...
                        html += `<tr><td style="font-weight:bold;color:${i.puntaje>=700?'#27ae60':'#c0392b'}">${i.puntaje}</td><td>${d.toLocaleDateString()}</td><td>${d.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</td></tr>`;
                    });
                    html += `</tbody></table></div>`;
                });
            }
            card.innerHTML = `<div class="user-header"><div style="text-align:left;"><h3>${user.nombre}</h3><small>${user.ciudad}</small></div><div style="display:flex;align-items:center;"><button class="btn-pdf-mini"><i class="fas fa-file-pdf"></i> PDF</button><div style="text-align:right;"><strong style="color:${intentosUser.length>0?'#b22222':'#999'};font-size:1.8rem;font-family:'Teko';">${intentosUser.length}</strong><span style="display:block;font-size:0.75rem;">TOTAL</span></div></div></div><div class="user-attempts">${html}</div>`;
    ...
    async function generatePDF(usersList, filename) {
        ...
        for (const u of usersList) {
            ...
            for (const m of mats) {
                ...
                stat(doc, 140, 45, "PROMEDIO", prom, 178,34,34); stat(doc, 170, 45, "MEJOR", max, 39,174,96);
                ...
                doc.autoTable({ head:[['#','Nota','Fecha','Hora']], body:rows, startY:155, theme:'grid', headStyles:{fillColor:[30,30,30]} });
            }
        }
        doc.save(filename);
    }
    ...
    async function getChart(data) {
        return new Promise(r => {
            ...
            window.myChart = new Chart(ctx, {
                type: 'bar', data: { labels: data.map((_,i)=>i+1), datasets: [{ data: data.map(i=>i.puntaje), backgroundColor: data.map(i=>i.puntaje>=700?'#27ae60':'#b22222') }] },
